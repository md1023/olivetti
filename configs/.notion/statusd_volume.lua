...
local function get_volume()
   ...
   local ml_index_start, ml_index_end = string.find(s, "%[%d*%%%]")
   local master_level = string.sub(s, ml_index_start+1, ml_index_end-1)

   local ms_index_start, ms_index_end = string.find(s, "%[%a*%]")
   local master_state = string.sub(s, ms_index_start+1, ms_index_end-1)
